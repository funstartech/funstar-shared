...
func CreateOrder(ctx context.Context, orderID uint64,
	summary string, price int32, callbackPath string, expireSeconds uint32) (*CreateOrderRsp, error) {
	...
	req := &CreateOrderReq{
		...
		TimeExpire:     now.Add(time.Second * time.Duration(expireSeconds)).Format(timeFormat),
		TradeType:      "JSAPI",
		Openid:         openid,
	}
	rsp := &CreateOrderRsp{}
